...
-- Switch collector to use our own proxy (no geo-block) and remove simulation fallback
...
DECLARE
  ...
  v_ts timestamptz;
BEGIN
  -- Process recent successful responses from our own proxy
  FOR resp IN
    SELECT id, content
  ...
  -- Trigger next request through our own proxy (Cloudflare worker not geo-blocked)
  PERFORM net.http_get(
    url := 'https://historicofreitas.lovable.app/api/public/recent',
...
-- Remove all simulated rows so the history shows only real Blaze data
